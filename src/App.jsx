...
function App() {
  ...
  // Obtener ranking de TOP APTs por correlación de TTPs de infraestructura
  ...
  // Helper para determinar la clase CSS del ranking
  ...
  useEffect(() => {
    ...
    const visNodesArray = graphData.nodes
      .filter(node => {
        // Excluir TTPs y ThreatActors del mapa visual
        if (node.labels.includes('TTP') || node.labels.includes('ThreatActor')) {
          return false
        }
      ...
      <div className="dashboard-wrapper">
        ...
        <header className="dashboard-header">
          ...
          <div className="header-actions">
            <button className="apt-btn-header" onClick={fetchTopAPTs}>
              🎯 TOP APTs
            </button>
            ...
            <div className="apt-panel">
              ...
              <div className="apt-panel-body">
                ...
                {!aptLoading && !aptError && aptData.map((apt, index) => (
                  <div key={apt.actor_id} className={`apt-card ${getRankClass(index)}`}>
                    <div className="apt-card-top">
                      <div className="apt-card-rank">#{index + 1}</div>
                      <div className="apt-card-info">
                        <h3 className="apt-card-name">{apt.actor_name}</h3>
                      ...
                      <div className="apt-card-coverage">
                        <div className="apt-coverage-value">{apt.coverage_percent}%</div>
                        <div className="apt-coverage-label">{apt.matched_ttp_count} / {apt.total_infra_ttps} TTPs</div>
                      </div>
                    </div>

                    <div className="apt-progress-bar">
                      <div className="apt-progress-fill" style={{ width: `${apt.coverage_percent}%` }}></div>
                    </div>

                    <div className="apt-ttp-chips">
                      {apt.matched_ttp_ids && apt.matched_ttp_ids.map((ttpId, i) => (
                        <span key={ttpId} className="apt-ttp-chip">
                          <span className="chip-id">{ttpId}</span>
                          {apt.matched_ttp_names && apt.matched_ttp_names[i]}
                        </span>
                      ))}
                    </div>
                  </div>
