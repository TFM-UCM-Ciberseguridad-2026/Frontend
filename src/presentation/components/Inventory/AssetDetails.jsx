...
export function AssetDetails({ selectedNode }) {
  if (!selectedNode) {
    return (
  ...
  const isVuln = selectedNode.categoryId === 'vulnerabilidad' || selectedNode.primaryLabel === 'Vulnerability';
      ...
      <div style={{ padding: '10px 0' }}>
        ...
        <div className="props">
          <div className="prop-row">
            ...
            <div className="v" style={{ fontFamily: 'Share Tech Mono, monospace' }}>{selectedNode.id}</div>
          </div>
          {Object.entries(selectedNode.properties || {}).map(([k, v]) => {
            ...
            return (
              <div className="prop-row" key={k}>
                ...
                <div className="v" style={{ wordBreak: 'break-all' }}>{display}</div>
              </div>
            );
          })}
        </div>
